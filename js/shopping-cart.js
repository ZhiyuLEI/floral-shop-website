...
{
	//Load the XML file that contains the information for all the Flowers
	xmlDoc=loadXMLFile("Flowers.xml");

    //Construct a table that displays the key information for all the Flowers
	txt="<table border='0px' id='tblFlower' cellpadding='3px' cellspace='0px'>";
	txt+="<tr><th>Item</th><th>Title</th><th>Origin</th><th>Price</th><th>Add</th></tr>";
		
	//Retrieve all the Flower elements from the XML file
    flowers=xmlDoc.documentElement.getElementsByTagName("FLOWER");
    for (i=0;i<flowers.length;i++)
    {
      txt+="<tr>";
      
	  txt+="<td align='center'>" + i + "</td>";
	  FlowerChildren=flowers[i].getElementsByTagName("ITEM");
      txt+="<td align='center' id='title"+ i +"' width ='150px'>" + FlowerChildren[0].firstChild.nodeValue + "</td>"; 
         
      FlowerChildren=flowers[i].getElementsByTagName("ORIGIN");
      txt+="<td align='center' width ='150px'>" + FlowerChildren[0].firstChild.nodeValue + "</td>";
	  
	  FlowerChildren=flowers[i].getElementsByTagName("PRICE");
      txt+="<td align='center' id='price"+ i +"'width ='150px'>" + FlowerChildren[0].firstChild.nodeValue + "</td>";
	  
	  txt+="<td align='center'><img src='images/Flower-icon.jpg' onclick='addToCart(" + i + ");'/>"  + "</td>";

      txt=txt + "</tr>";
    }
    txt+="</table>";
	
	//Display Flower table in <div id="txtFlowerInfo">
    document.getElementById("txtFlowerInfo").innerHTML=txt;
}
